        ...
        <div className="min-h-screen bg-slate-50 py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-3xl mx-auto space-y-8">
                ...
                <div className="text-center space-y-2">
                    <h1 className="text-3xl font-bold text-slate-900">{interview.title}</h1>
              ...
              <div className="flex items-center space-x-2 text-sm text-slate-600 bg-white p-2 rounded border border-slate-200">
                <span>Difficulty : </span>
                            <span className={`px-2 py-0.5 rounded text-xs font-medium border
                                ${interview.difficulty === 'Easy' ? 'bg-green-50 text-green-700 border-green-200' :
                                    interview.difficulty === 'Medium' ? 'bg-yellow-50 text-yellow-700 border-yellow-200' :
                ...
                <div className="space-y-6">
                    {/* Instructions */}
                    <div>
                        <h2 className="text-lg font-semibold text-slate-900 mb-4">Before you begin:</h2>
                    ...
                    {/* Permissions Section */}
                    <div>
                        <h2 className="text-lg font-semibold text-slate-900 mb-4">Required Permissions:</h2>
                        <div className="grid md:grid-cols-2 gap-4">
                            <div className={`p-4 rounded-lg border flex items-center space-x-4 transition-colors
                                ${cameraActive ? 'bg-green-50 border-green-200' : 'bg-white border-slate-200'}`}>
                                <div className={`p-2 rounded-full ${cameraActive ? 'bg-green-100' : 'bg-slate-100'}`}>
                                    <Video className={`w-5 h-5 ${cameraActive ? 'text-green-600' : 'text-slate-500'}`} />
                                </div>
                                <div>
                                    <p className="font-medium text-slate-900">Camera</p>
                                    <p className="text-xs text-slate-500">{cameraActive ? 'Access granted' : 'Required'}</p>
                                </div>
                                {cameraActive && <CheckCircle2 className="w-5 h-5 text-green-600 ml-auto" />}
                            </div>

                            <div className={`p-4 rounded-lg border flex items-center space-x-4 transition-colors
                                ${micActive ? 'bg-green-50 border-green-200' : 'bg-white border-slate-200'}`}>
                                <div className={`p-2 rounded-full ${micActive ? 'bg-green-100' : 'bg-slate-100'}`}>
                                    <Mic className={`w-5 h-5 ${micActive ? 'text-green-600' : 'text-slate-500'}`} />
                                </div>
                                <div>
                                    <p className="font-medium text-slate-900">Microphone</p>
                                    <p className="text-xs text-slate-500">{micActive ? 'Access granted' : 'Required'}</p>
                                </div>
                                {micActive && <CheckCircle2 className="w-5 h-5 text-green-600 ml-auto" />}
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <h2 className="text-lg font-semibold text-slate-900 mb-4">Resume (Optional):</h2>
                ...
                <label htmlFor="resume-upload" className="cursor-pointer flex flex-col items-center justify-center">
                  {resumeUploading ? (
                      <div className="flex flex-col items-center">
                        ...
                        <p className="font-medium text-slate-900">Parsing Resume...</p>
                        <p className="text-xs text-slate-500 mt-1">Our AI is extracting your details</p>
                      </div>
                  ) : resumeFile ? (
                                    <>
                                    ...
                                    <>
                                        ...
                                        <p className="font-medium text-slate-900">Upload your Resume (PDF)</p>
                    ...
                    <Button
                        ...
                        onClick={permissionsGranted ? handleStartInterview : requestPermissions}
                    >
                        {permissionsGranted ? (
                            <>
                                Start Interview
